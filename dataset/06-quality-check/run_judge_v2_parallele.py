# v2: PARALLELE METRIC, SINGLE DATA AT TIME
...
# -----------------------------------------------
# METRIC EVALUATION (* parallel here)
# -----------------------------------------------
def evaluate_sample(sample):
    ...
    def run_metric(key, prompt):
        output = call_llm(prompt)
        parsed = parse_json(output) if output else None

        if parsed:
            return key, parsed.get("score", -1), parsed.get("reason", "")
        else:
            return key, -1, "parse_error"

    # parallel execution
    with ThreadPoolExecutor(max_workers=12) as executor:
        futures = [executor.submit(run_metric, k, p) for k, p in prompts.items()]
    ...
    return {
        "scores": scores,
        "reasons": reasons,
        "judge_id": "automated-review-by:gpt"
    }
