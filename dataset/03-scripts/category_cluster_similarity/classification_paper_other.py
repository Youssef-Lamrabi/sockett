...
# en se basant sur N rows déjà classifiés correctement.
#
# Méthode :
...
#   3. Embedder les 25 clusters (moyenne des keywords)
...
def main():
    ...
    parser.add_argument("--input", default="paper_non_other.jsonl",
    ...
    # 3. Embedder les 25 clusters
    print("Embedding des clusters...")
    cluster_texts      = [" ".join(CATEGORY_CLUSTERS[cat]) for cat in CATEGORIES]
    ...
    for i, row in enumerate(sample):
        ...
        match = "✅" if rank == 1 else f"❌ (prédit: {best_cat} @ {best_score:.3f})"
