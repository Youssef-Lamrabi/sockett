...
cleanup_old_installations() {
    ...
    local old_home_dir="$HOME/bioagent_tools"
    ...
    PATH=$(echo $PATH | tr ':' '\n' | grep -v "bioagent_tools/bin" | tr '\n' ':' | sed 's/:$//')
    export PATH="$TOOLS_DIR/bin:$PATH"
    echo -e "${GREEN}PATH cleaned up.${NC}"
}

# Create a directory for CLI tools if it doesn't exist
if [ -n "$BIOAGENT_TOOLS_DIR" ]; then
    TOOLS_DIR="$BIOAGENT_TOOLS_DIR"
else
    # TODO: For review
    TOOLS_DIR="$(pwd)/bioagent_tools"
fi
...
echo "# Added by bioagent setup" >> "$TOOLS_DIR/setup_path.sh"
    ...
    if [ -n "$profile_file" ]; then
        ...
        if [ -w "$profile_file" ]; then
            ...
            if ! grep -q "export PATH=\"$TOOLS_DIR/bin:\$PATH\"" "$profile_file"; then
                # Remove any old bioagent_tools paths first
                if grep -q "bioagent_tools/bin" "$profile_file"; then
                    echo -e "${YELLOW}Removing old bioagent_tools paths from $profile_file...${NC}"
                    sed -i '/bioagent_tools\/bin/d' "$profile_file"
                fi

                echo "" >> "$profile_file"
                echo "# Added by bioagent setup" >> "$profile_file"
...
install_cli_tools() {
    ...
    if [ -n "$BIOAGENT_AUTO_INSTALL" ]; then
        install_all_tools 1
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    # Set BIOAGENT_AUTO_INSTALL if it's not already set
    export BIOAGENT_AUTO_INSTALL=1
    ...
    exit 0
elif [ -n "$BIOAGENT_AUTO_INSTALL" ]; then
    # Run in auto-install mode without showing the menu
    install_all_tools 1
