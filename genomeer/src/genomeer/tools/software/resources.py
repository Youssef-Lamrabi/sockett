...
library_content_dict = {
    ...
    # ── NEW METAGENOMICS TOOLS ────────────────────────────────────────────────
    "das_tool": (
        "[CLI Tool][TIMEOUT: 3600s] DAS_Tool: bin dereplication and refinement from multiple binners.\n"
        "  # Step 1: build scaffold-to-bin files for each binner\n"
        "  subprocess.run(['Fasta_to_Scaffolds2Bin.sh', '-i', 'metabat_bins/', '-e', 'fa'],\n"
        "                 stdout=open('metabat.s2b', 'w'), timeout=120)\n"
        "  subprocess.run(['Fasta_to_Scaffolds2Bin.sh', '-i', 'maxbin_bins/', '-e', 'fasta'],\n"
        "                 stdout=open('maxbin.s2b', 'w'), timeout=120)\n"
        ...
        "  # Columns: bin_id, unique_SCGs, redundant_SCGs, SCG_completeness (0-1), SCG_redundancy (0-1)\n"
        "  # Refined bins: dastool_out/dastool_DASTool_bins/*.fa\n"
        "  # --score_threshold: 0.5 default; lower to 0.35 to recover more bins at lower confidence\n"
    ),

    "bracken": (
        "[CLI Tool][TIMEOUT: 300s] Bracken: Bayesian re-estimation of species abundance from Kraken2 report.\n"
        "  # Requires Bracken db built alongside Kraken2 db (bracken-build or pre-built)\n"
        "  res = subprocess.run(['bracken',\n"
        "                        '-d', 'kraken2_db/',   # same path as Kraken2 database\n"
        "                        '-i', 'report.txt',    # Kraken2 --report output\n"
        "                        '-o', 'bracken_output.bracken',\n"
        "                        '-r', '150',           # read length (adapt: 75, 100, 150, 250)\n"
        "                        '-l', 'S',             # level: S=species G=genus P=phylum\n"
        "                        '-t', '10'],           # min reads threshold for re-estimation\n"
        "                       capture_output=True, text=True, timeout=300)\n"
        "  # Output TSV: name, taxonomy_id, taxonomy_lvl, kraken_assigned_reads,\n"
        "  #             added_reads, new_est_reads, fraction_total_reads\n"
        "  import pandas as pd\n"
        "  df = pd.read_csv('bracken_output.bracken', sep='\\t')\n"
        "  top10 = df.sort_values('fraction_total_reads', ascending=False).head(10)\n"
    ),

    "metaphlan4": (
        "[CLI Tool][TIMEOUT: 3600s] MetaPhlAn4: marker-gene taxonomic profiling of metagenomes.\n"
        "  # Single-end:\n"
        "  res = subprocess.run(['metaphlan', 'reads.fastq.gz',\n"
        "                        '--input_type', 'fastq',\n"
        "                        '--bowtie2db', 'metaphlan4_db/',\n"
        "                        '-o', 'profile.tsv', '--nproc', '4'],\n"
        "                       capture_output=True, text=True, timeout=3600)\n"
        "  # Paired-end:\n"
        "  res = subprocess.run(['metaphlan', 'r1.fastq.gz,r2.fastq.gz',\n"
        "                        '--input_type', 'fastq', '--bowtie2db', 'metaphlan4_db/',\n"
        "                        '-o', 'profile.tsv', '--nproc', '4'],\n"
        "                       capture_output=True, text=True, timeout=3600)\n"
        "  # Output TSV: #clade_name, NCBI_tax_id, relative_abundance, coverage, estimated_reads\n"
        "  # Species rows contain 's__' but not 't__'\n"
        "  import pandas as pd\n"
        "  df = pd.read_csv('profile.tsv', sep='\\t', comment='#',\n"
        "                   names=['clade','taxid','rel_abund','coverage','est_reads'])\n"
        "  species = df[df['clade'].str.contains('s__') & ~df['clade'].str.contains('t__')]\n"
    ),

    "gtdbtk": (
        "[CLI Tool][TIMEOUT: 14400s] GTDB-Tk: phylogenetic classification of MAGs against GTDB.\n"
        "  import os\n"
        "  os.environ['GTDBTK_DATA_PATH'] = '/path/to/gtdbtk_data/'  # set GTDB database path\n"
        "  res = subprocess.run(['gtdbtk', 'classify_wf',\n"
        "                        '--genome_dir', 'bins_dir/',\n"
        "                        '--out_dir', 'gtdbtk_out/',\n"
        "                        '--extension', 'fa',      # match your bin extension: fa/fna/fasta\n"
        "                        '--cpus', '4',\n"
        "                        '--skip_ani_screen'],     # skip for speed on small datasets\n"
        "                       capture_output=True, text=True, timeout=14400)\n"
        "  # Output bacteria: gtdbtk_out/gtdbtk.bac120.summary.tsv\n"
        "  # Output archaea:  gtdbtk_out/gtdbtk.ar53.summary.tsv\n"
        "  # Key columns: user_genome, classification, msa_percent, red_value, warnings\n"
        "  # msa_percent >= 50% = reliable phylogenetic placement (GTDB-Tk criterion)\n"
        "  import pandas as pd, glob as gl\n"
        "  files = gl.glob('gtdbtk_out/gtdbtk.*.summary.tsv')\n"
        "  df = pd.concat([pd.read_csv(f, sep='\\t') for f in files], ignore_index=True)\n"
    ),

    "prokka": (
        "[CLI Tool][TIMEOUT: 3600s] Prokka: rapid prokaryotic genome annotation.\n"
        "  # For a single bin or genome:\n"
        "  res = subprocess.run(['prokka', '--outdir', 'prokka_out/', '--prefix', 'sample',\n"
        "                        '--cpus', '4',\n"
        "                        '--metagenome',   # use for bins (disables rRNA prediction)\n"
        "                        '--kingdom', 'Bacteria',\n"
        "                        'bin.fna'],\n"
        "                       capture_output=True, text=True, timeout=3600)\n"
        "  # Output: prokka_out/sample.gff, .gbk, .faa (proteins), .ffn (genes), .tsv\n"
        "  # sample.tsv: locus_tag, ftype, len_bp, gene, EC_number, COG, product\n"
        "  import pandas as pd\n"
        "  ann = pd.read_csv('prokka_out/sample.tsv', sep='\\t')\n"
        "  print(f'Predicted genes: {len(ann)}')\n"
        "  # For plain assembly (not bins): omit --metagenome\n"
    ),

    "medaka": (
        "[CLI Tool][TIMEOUT: 7200s] Medaka: nanopore consensus polishing for ONT assemblies.\n"
        "  # medaka_consensus wraps Racon + Medaka in one command:\n"
        "  res = subprocess.run(['medaka_consensus',\n"
        "                        '-i', 'reads.fastq',         # ONT reads (fastq)\n"
        "                        '-d', 'draft_assembly.fna',  # Flye/Raven draft\n"
        "                        '-o', 'medaka_out/',\n"
        "                        '-t', '4',\n"
        "                        '-m', 'r941_min_high_g360'], # model: match your flowcell+kit\n"
        "                       capture_output=True, text=True, timeout=7200)\n"
        "  # Common models: r941_min_high_g360, r941_min_sup_g507, r1041_e82_400bps_sup_g615\n"
        "  # List available: medaka tools list_models\n"
        "  # Output: medaka_out/consensus.fasta\n"
        "  import os\n"
        "  consensus = 'medaka_out/consensus.fasta'\n"
        "  assert os.path.exists(consensus) and os.path.getsize(consensus) > 0, 'Medaka failed'\n"
    ),

    "rgi": (
        "[CLI Tool][TIMEOUT: 1800s] RGI (Resistance Gene Identifier): AMR gene detection from CARD database.\n"
        "  # Load CARD database (run once after downloading card.json):\n"
        "  # subprocess.run(['rgi', 'load', '--card_json', 'card.json', '--local'], ...)\n"
        "  # Run on protein sequences:\n"
        "  res = subprocess.run(['rgi', 'main',\n"
        "                        '-i', 'proteins.faa',   # protein FASTA (or use contigs.fna with -t contig)\n"
        "                        '-o', 'rgi_output',     # prefix — output becomes rgi_output.txt\n"
        "                        '-t', 'protein',        # protein OR contig\n"
        "                        '-a', 'BLAST',          # alignment: BLAST (accurate) or DIAMOND (fast)\n"
        "                        '--local', '--clean', '-n', '4'],\n"
        "                       capture_output=True, text=True, timeout=1800)\n"
        "  # Output: rgi_output.txt TSV\n"
        "  # Key column: Cut_Off — Perfect (100% id), Strict (above identity cutoff), Loose (low confidence)\n"
        "  # Best practice: report only Perfect + Strict hits (Loose has high false-positive rate)\n"
        "  import pandas as pd\n"
        "  df = pd.read_csv('rgi_output.txt', sep='\\t')\n"
        "  high_conf = df[df['Cut_Off'].isin(['Perfect', 'Strict'])]\n"
        "  print(f'High-confidence AMR genes: {len(high_conf)}')\n"
    ),

    "amrfinder": (
        "[CLI Tool][TIMEOUT: 600s] AMRFinderPlus: NCBI AMR gene detection with curated database.\n"
        "  # Update database first (run once): amrfinder -u\n"
        "  res = subprocess.run(['amrfinder',\n"
        "                        '-p', 'proteins.faa',  # protein (-p) or nucleotide (-n)\n"
        "                        '-o', 'amr_results.tsv',\n"
        "                        '--plus',              # include stress, virulence, and point mutations\n"
        "                        '-O', 'Escherichia',   # organism for point mutation detection (optional)\n"
        "                        '--threads', '4'],\n"
        "                       capture_output=True, text=True, timeout=600)\n"
        "  # Output TSV columns: Gene symbol, Sequence name, Scope, Element type, Subtype,\n"
        "  #   Class, Subclass, % Coverage of reference sequence, % Identity to reference sequence\n"
        "  # NCBI high-confidence criterion: % Coverage >= 90% AND % Identity >= 90%\n"
        "  import pandas as pd\n"
        "  df = pd.read_csv('amr_results.tsv', sep='\\t')\n"
        "  high_conf = df[(df['% Coverage of reference sequence'] >= 90) &\n"
