...
description = [
    ...
    },
    {
        ...
        "description": (
            ...
            "subprocess stdout=). AFTER running, ALWAYS check returncode==0 AND that the output file is "
            "NON-EMPTY (os.path.getsize>0) — filtlong ABORTS (writing nothing → 0-byte file) when input "
            "read NAMES are duplicated, e.g. after `cat`-ing paired _1/_2 FASTQs (real failure "
            "SRR8359173: 'Error: duplicate read name'). NEVER concatenate paired _1/_2 files here — their "
            "presence means the run is short-read ILLUMINA (wrong platform for long-read assembly); use a "
            "genuine single ONT/PacBio FASTQ instead. Also, if EVERY read is < --min_length (e.g. 150 bp "
            "Illumina reads through --min_length 1000) the output is empty — that too signals wrong-platform data. "
            ...
            "MANDATORY SUBSAMPLING for a single ISOLATE assembly: ALWAYS pass --target_bases to cap "
            "coverage to ~100x, else a deep run makes Flye run for HOURS (real failure run-217: a 2.4 GB "
            "ONT filtered set = ~480x for a 5 Mb genome → Flye still assembling after 98 min). Set "
            "--target_bases = 100 * expected_genome_bp (bacteria ≈ 5,000,000 → --target_bases 500000000). "
            "filtlong keeps the BEST reads up to that budget, so quality stays high while the assembler "
            "runs in minutes. For a metagenome (many genomes) do NOT cap this aggressively. "
