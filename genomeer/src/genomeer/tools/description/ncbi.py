description = [
    ...
    },
    {
        ...
        "description": (
            ...
            "    The runinfo CSV has: col 1 = Run, col 8 = size_MB, col 13 = LibraryStrategy, col 16 = LibraryLayout.\n"
            "    RUN SELECTION (critical — do NOT just take the 'first PAIRED run'): a BioProject often MIXES\n"
            "    data types, so you MUST select the run that MATCHES THE TASK by reading LibraryStrategy:\n"
            "      - metagenomic assembly / MAG reconstruction -> require LibraryStrategy = WGS or METAGENOMIC or\n"
            "        OTHER; EXCLUDE RNA-Seq / AMPLICON / ChIP-Seq / Bisulfite-Seq — the WRONG data type gives\n"
            "        biologically GARBAGE MAGs (RNA-Seq has no even genome coverage; amplicon is one marker).\n"
            "      - prefer the biggest suitable run for depth; paired is nicer but a single-end WGS run is still\n"
            "        valid for assembly (co-assemble several small WGS runs if each is tiny).\n"
            "    RECONCILE WITH THE PROMPT'S CLAIM: if the prompt says e.g. 'paired-end shotgun metagenomic ~15GB'\n"
            "    but NO run matches (REAL case PRJNA288120: the WGS runs are single-end & tiny, the only PAIRED\n"
            "    runs are RNA-Seq), do NOT silently download the wrong thing. STATE the mismatch explicitly, pick\n"
            "    the closest VALID metagenomic run(s) (co-assembling single-end WGS if needed), and flag it as an\n"
            "    artifact caveat. The prompt's description of a dataset is a CLAIM to verify against runinfo, not\n"
            "    a fact — the runinfo LibraryStrategy is authoritative.\n"
            "    DISK GUARD: before wget, check the run's fastq size (ENA filereport fastq_bytes, or size_MB*~4 for\n"
            "    decompressed) vs FREE disk (df). If free disk < ~3x the expected decompressed size, STOP and\n"
            "    report rather than filling the disk (a full disk breaks the whole machine).\n"
