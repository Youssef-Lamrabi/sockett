...
description = [
    ...
    # ── BIN QUALITY ───────────────────────────────────────────────────────────
    {
        ...
        "description": (
            ...
            "⚠ ALWAYS read the 'Additional_Notes' column too, not just Completeness/Contamination — "
            "CheckM2 uses it to flag its OWN low-confidence predictions, e.g. 'Low confidence prediction - "
            "substantial (43%) disagreement between completeness prediction models'. When this note is "
            "present, the completeness/contamination numbers are UNCERTAIN (the two internal ML models "
            "disagree) — report this caveat explicitly next to the numbers; do NOT present them as solid "
            "ground truth, and do NOT attribute a low completeness solely to contamination/other causes "
            "without first surfacing that CheckM2 itself flagged the estimate as unreliable."
        ),
        "required_parameters": [
    ...
    # ── TAXONOMIC CLASSIFICATION ──────────────────────────────────────────────
    {
        ...
        "description": (
            ...
            "⚠ CONTAMINATION SANITY CHECK (real failure — verified): before concluding that N contigs are "
            "'contamination from species X' (e.g. Homo sapiens in a bacterial assembly), CHECK the GC% of "
            "those SPECIFIC contigs (seqkit fx2tab -g, or simple base counting) against the KNOWN GC of "
            "species X — human is ~41% GC, most bacteria 30-70% depending on genus. If the flagged contigs' "
            "GC is wildly outside ANY plausible genomic range (e.g. <10% or >80% — biologically near-"
            "impossible for real genomic DNA of ANY organism), they are NOT real contamination — they are "
            "most likely LOW-COMPLEXITY ASSEMBLY ARTIFACTS (homopolymer-collapse from high-error-rate raw "
            "long reads, degenerate/repeat regions) that kraken2 spuriously matched because their degenerate "
            "k-mer content doesn't cleanly match anything, not because the sample truly contains species X. "
            "Report the compositional evidence either way (e.g. 'contig has GC=2.8%, inconsistent with real "
            "human or bacterial DNA — likely an assembly artifact, not contamination') instead of taking the "
            "raw kraken2 per-contig label at face value."
        ),
        "required_parameters": [
    ...
    },
    {
        ...
        "description": (
            ...
            "⚠ ON 'ERROR *** Possible organisms: ...' (exit 1) — the species you passed is simply NOT in "
            "AMRFinderPlus's curated organism list (this is COMMON: most species, e.g. Pseudomonas fulva, "
            "are not covered — it is NOT a bug and NOT fixable by guessing a different name). There is NO "
            "genus-level fallback (e.g. '--organism Pseudomonas' is INVALID and will fail identically). "
            "FIX: immediately re-run the EXACT same command WITHOUT -O/--organism at all — AMRFinderPlus "
            "still reports the full AMR/stress/virulence gene screen with --plus, it only skips the "
            "species-specific point-mutation calls. This retry is a COMPLETE, valid result, not a "
            "degraded one — report it as such, and do not suggest re-trying with a different organism "
            "name or a genus name in the final report; state plainly that point-mutation screening was "
            "skipped because the species has no curated AMRFinderPlus reference panel. "
    ...
    },
    {
        ...
        "description": (
            ...
            "FLAG NAME (real bug): use the SHORT flag `-m`, or the LONG flag `--methods` (PLURAL). "
            "`--method` (singular) does NOT exist → CoverM aborts with \"unexpected argument '--method'\". "
    ...
    },
    {
        ...
        "description": (
            ...
            "NOTE (real bug): `genome_wide` does NOT accept `-o` (it writes INTO the -i IS directory); "
            "passing `-o <dir>` aborts with \"unrecognized arguments: -o\". Parse per-genome popANI from "
            "`cmp.IS/output/*_genomeWide_compare.tsv` (NOT *_comparisonsTable.tsv, which has no `genome` col). "
            ...
            "⚠ CRITICAL — FRAGMENTED CO-ASSEMBLY COVERAGE COLLAPSE (real, verified failure): on a "
            "metagenomic co-assembly with many SHORT contigs (a few hundred to a few kb, typical of "
            "MEGAHIT/SPAdes output), inStrain's DEFAULT `pairing_filter='paired'` requires BOTH mates of "
            "a read pair to land on the SAME scaffold within a plausible insert size — but on short "
            "contigs a large fraction of pairs straddle a contig boundary (one mate per contig), so "
            "BOTH reads of those pairs are silently dropped from profiling. This makes inStrain's own "
            "*_genome_info.tsv / *_scaffold_info.tsv `coverage` column read up to 10,000x LOWER than the "
            "TRUE depth (verified: samtools/jgi_summarize_bam_contig_depths reported a healthy median "
            "~17-46x on the exact same BAM+reference, while inStrain reported ~0.001-0.005x for the same "
            "scaffolds) — this looks like 'the community has almost no coverage' but is actually an "
            "ARTIFACT of the pairing filter on a fragmented reference, not real biology. FIX: pass "
            "`--pairing_filter all_reads` to `inStrain profile` whenever the reference is a multi-contig "
            "co-assembly (not a single closed genome) — this counts every mapped read regardless of its "
            "mate's placement. SANITY CHECK (always do this): after profiling, compare inStrain's "
            "reported coverage for a few scaffolds against the SAME scaffolds' depth in "
            "jgi_summarize_bam_contig_depths' depth.txt (or `samtools depth`) — if inStrain's number is "
            "orders of magnitude lower, the pairing filter is the cause; do not conclude 'coverage too "
            "low for strain analysis' without this cross-check first."
        ),
        "required_parameters": [
    ...
    },
    {
        ...
        "description": (
            ...
            "Outputs in out_dir: contig_report.txt (per-contig chromosome/plasmid + cluster) is "
            "ALWAYS written, regardless of outcome — check ONLY this file for success. "
            "⚠ mobtyper_results.txt AND plasmid_*.fasta are CONDITIONAL — mob_recon writes them ONLY "
            "if it found at least one plasmid-classified contig; a genome with NO plasmids (every "
            "contig's molecule_type == 'chromosome' in contig_report.txt) is a real, valid, COMPLETE "
            "result and correctly produces NEITHER file. REAL FAILURE this causes: code that does "
            "`if not os.path.exists(mobtyper_results): sys.exit(...)` treats a legitimate "
            "plasmid-free genome as a crashed run, discards the (perfectly good) contig_report.txt, "
            "and the retry/fallback that follows often skips parsing entirely — reporting 'mob_recon "
            "output not captured' even though contig_report.txt was sitting on disk the whole time. "
            "CORRECT CHECK: only require contig_report.txt to exist; count plasmids by reading it "
            "(molecule_type=='plasmid' rows / distinct primary_cluster_id) or by counting "
            "plasmid_*.fasta if present — glob.glob() on missing files returns an empty list, so no "
            "existence check is even needed for the optional outputs. Report 'no plasmid detected, "
            "all resistance/virulence genes are chromosomal' as a first-class, non-error conclusion "
            "when the count is 0. "
    ...
    },
    {
        ...
        "description": (
            ...
            "threshold 95% ANI). "
            "Command: dRep dereplicate OUT_DIR -g <file1.fa> <file2.fa> ... -p 4 --genomeInfo genomeInfo.csv "
            "-sa 0.95 -comp 50 -con 10 . "
            "CRITICAL — `-g` MUST receive an EXPLICIT LIST of file paths, NOT a shell glob. When you call "
            "dRep via subprocess.run([...]) there is NO shell, so `-g bins/*.fa` (or a directory) is passed "
            "as a LITERAL string and dRep fails/loads nothing. In Python do "
            "`bins = glob.glob(os.path.join(bins_dir, '*.fa'))` and splat them: "
            "`subprocess.run(['dRep','dereplicate',out_dir,'-g',*bins,'-p','4','--genomeInfo',info_csv, ...])`. "
