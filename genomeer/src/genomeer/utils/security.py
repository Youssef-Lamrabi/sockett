...
def check_bash_script(script: str) -> Tuple[bool, str]:
    ...
    Parameters
    ----------
    script : Contenu du script bash à vérifier.

    Returns
    -------
    (is_safe: bool, reason: str)
      is_safe = True si aucun pattern dangereux détecté.
      reason  = "ok" si safe, message d'erreur sinon.
    """
...
def check_python_code(code: str) -> Tuple[bool, str]:
    ...
    Parameters
    ----------
    code : Contenu Python à vérifier.
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                ...
                if func_name in ("eval", "exec", "compile", "__import__", "getattr", "setattr", "hasattr", "delattr"):
