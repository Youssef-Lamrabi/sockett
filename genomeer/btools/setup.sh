...
# BioAgentOS - Btools Environment Setup Script
...
echo -e "${YELLOW}=== Btools Environment Setup ===${NC}"
...
main() {
    ...
    conda env create -n btools_env1 -f environment.yml
    ...
    if command -v micromamba &> /dev/null; then
        ...
        micromamba activate btools_env1
    else
        eval "$(conda shell.bash hook)"
        conda activate btools_env1
    fi
    handle_error $? "Failed to activate btools_env1 environment."
    ...
    echo -e "To activate this environment in the future, run: ${YELLOW}conda activate btools_env1${NC}"
